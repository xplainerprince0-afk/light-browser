...
object AlpineEnv {
    ...
    private const val B_FUNCTION =
        "# >>> LIGHTBROWSER-B (managed — do not edit) >>>\n" +
            ...
            "  _b_c=\"\$1\"; [ \$# -gt 0 ] && shift\n" +
            "  case \"\$_b_c\" in\n" +
            "    ''|help) echo 'b open|new|tabs|tab|close|home|back|forward|reload|reload-hard|stop|url|title|ua|viewport|zoom|find|next|prev|snap|shot-el|text|read|dom|js|links|forms|netlog|wait|survey|shot|save|metrics|console|cookies|clear-data|tabdup|history|downloads|click|fill|submit|key|hover|select|store|stores|unstore|alias|mkext|ext|mkcmd|cmds|blocks|pos|tap|swipe|scroll|scrollto|scroll-top|scroll-bottom|serve|record|run (server must be on; do/replay/queue/unblock/block are EXEC-only)';;\n" +
